...
pub fn gradient_blend(base: &Grid<Tile>, overlay: &Grid<Tile>, output: &mut Grid<Tile>, horizontal: bool) {
    ...
    for y in 0..h {
        for x in 0..w {
            ...
            let cell = if t > 0.5 { overlay[(x, y)] } else { base[(x, y)] };
            output.set(x as i32, y as i32, cell);
        }
    }
}

pub fn radial_blend(base: &Grid<Tile>, overlay: &Grid<Tile>, output: &mut Grid<Tile>, inner_r: f64, outer_r: f64) {
    let w = base.width().min(overlay.width()).min(output.width());
    let h = base.height().min(overlay.height()).min(output.height());
