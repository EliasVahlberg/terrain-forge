    ...
    for _ in 0..iterations {
        ...
        for y in 1..h - 1 {
            for x in 1..w - 1 {
                let idx = y * w + x;
                if snapshot[idx] {
                    let has_wall = !snapshot[idx - 1] || !snapshot[idx + 1] 
                        || !snapshot[idx - w] || !snapshot[idx + w];
                    if has_wall { grid.set(x as i32, y as i32, Tile::Wall); }
                }
            }
        }
    }
    ...
    for _ in 0..iterations {
        ...
        for y in 1..h - 1 {
            for x in 1..w - 1 {
                let idx = y * w + x;
                if !snapshot[idx] {
                    let has_floor = snapshot[idx - 1] || snapshot[idx + 1] 
                        || snapshot[idx - w] || snapshot[idx + w];
                    if has_floor { grid.set(x as i32, y as i32, Tile::Floor); }
                }
            }
        }
    }
