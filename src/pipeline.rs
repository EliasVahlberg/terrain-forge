...
pub enum PipelineCondition {
    ...
    FloorCount { min: Option<usize>, max: Option<usize> },
    /// Check if region count meets threshold  
    RegionCount { min: Option<usize>, max: Option<usize> },
        ...
        match self {
            PipelineCondition::FloorCount { min, max } => {
                ...
                if let Some(min_val) = min {
                    if count < *min_val { return false; }
                }
                if let Some(max_val) = max {
                    if count > *max_val { return false; }
                }
                true
            }
            PipelineCondition::RegionCount { min, max } => {
                let count = context.get_parameter("region_count")
            ...
            PipelineCondition::Density { min, max } => {
                ...
                if let Some(min_val) = min {
                    if density < *min_val { return false; }
                }
                if let Some(max_val) = max {
                    if density > *max_val { return false; }
                }
                true
            }
...
impl ParameterMap {
    ...
    pub fn add_branch(&mut self, branch_name: impl Into<String>, parameters: HashMap<String, String>) {
        self.branch_parameters.insert(branch_name.into(), parameters);
    }
    ...
    pub fn merge_all(&self) -> HashMap<String, String> {
        ...
        for (_, params) in &self.branch_parameters {
...
pub enum PipelineOperation {
    ...
    Effect { name: String, parameters: HashMap<String, String> },
...
impl ConditionalPipeline {
    ...
    pub fn execute(&self, grid: &mut Grid<Tile>, context: &mut PipelineContext, rng: &mut Rng) -> StageResult {
        ...
        if let Some(condition) = &op.condition {
            if condition.evaluate(grid, context) {
                ...
                for true_op in &op.if_true {
                    ...
                    result.output_parameters.extend(branch_result.output_parameters);
                }
            } else {
    ...
    fn execute_base_operation(
        ...
        match operation {
            ...
            PipelineOperation::Effect { name, parameters: _ } => {
...
impl PipelineTemplate {
    ...
    pub fn with_parameter(mut self, key: impl Into<String>, default_value: impl Into<String>) -> Self {
    ...
    pub fn instantiate(&self, custom_params: Option<HashMap<String, String>>) -> ConditionalPipeline {
    ...
    fn substitute_parameters(&self, operation: &ConditionalOperation, params: &HashMap<String, String>) -> ConditionalOperation {
        ...
        let sub_if_true: Vec<ConditionalOperation> = operation.if_true.iter()
            .map(|op| self.substitute_parameters(op, params))
            .collect();
        let sub_if_false: Vec<ConditionalOperation> = operation.if_false.iter()
            .map(|op| self.substitute_parameters(op, params))
            .collect();
...
impl TemplateLibrary {
    ...
    fn add_builtin_templates(&mut self) {
        ...
        let simple_dungeon = PipelineTemplate::new(
            "simple_dungeon",
            "Basic dungeon with rooms and corridors"
        )
        ...
        .with_operation(ConditionalOperation::conditional(
            ...
            PipelineCondition::Density { min: Some(0.1), max: Some(0.8) },
        ...
        let cave_system = PipelineTemplate::new(
            "cave_system",
            "Organic cave system with cellular automata"
        )
        ...
        // Maze template
        let maze_template = PipelineTemplate::new(
            "maze",
