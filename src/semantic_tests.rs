...
mod tests {
    ...
    fn test_room_accretion_semantic() {
        ...
        if let Some(semantic) = result.semantic {
            ...
            let rooms: Vec<_> = semantic.regions.iter()
                .filter(|r| r.kind == "room")
                .collect();
            assert!(!rooms.is_empty());
            
            // Should have diverse marker types
            let marker_types: std::collections::HashSet<_> = semantic.markers.iter()
                .map(|m| &m.tag)
                .collect();
            assert!(marker_types.len() > 1);
            
            println!("Room accretion generated {} rooms with marker types: {:?}", 
                     rooms.len(), marker_types);
        }
    }
