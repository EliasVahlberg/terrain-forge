...
fn main() {
    ...
    println!("   Generated {}x{} base layout with {} floors", 
             base_grid.width(), base_grid.height(), base_floors);
    ...
    println!("   Extracted {} unique 3x3 patterns", learned_patterns.len());
    ...
    println!("   WFC generated {} floors with learned patterns", wfc_floors);
    ...
    if room_centers.len() >= 3 {
        ...
        let total_length: f32 = mst_edges.iter()
    ...
    println!("     Basic BSP: {} floors ({:.1}%)", 
             basic_floors, 100.0 * basic_floors as f32 / (25 * 20) as f32);
    println!("     Enhanced (WFC + Prefabs): {} floors ({:.1}%)", 
             enhanced_floors, 100.0 * enhanced_floors as f32 / (50 * 20) as f32);
    ...
    println!("   2. Learned {} patterns for WFC enhancement", learned_patterns.len());
    println!("   3. Analyzed {} room connections with Delaunay triangulation", room_centers.len());
...
fn find_room_center(grid: &Grid<Tile>, visited: &mut [Vec<bool>], start_x: usize, start_y: usize) -> Point {
...
fn create_specialized_library() -> PrefabLibrary {
    ...
    let boss_room = PrefabData {
        ...
        weight: 0.2,
        tags: vec!["boss".to_string(), "special".to_string(), "large".to_string()],
    };
    ...
    let treasure_room = PrefabData {
        ...
        weight: 0.8,
        tags: vec!["treasure".to_string(), "special".to_string(), "small".to_string()],
    };
