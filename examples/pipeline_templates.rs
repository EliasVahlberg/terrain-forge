...
fn main() {
    ...
    println!("    Result: {}", if result1.success { "✅ Success" } else { "❌ Failed" });
    println!("    Dungeon type: {}", context1.get_parameter("dungeon_type").unwrap_or(&"unknown".to_string()));
    ...
    println!("    Dungeon type: {}", context2.get_parameter("dungeon_type").unwrap_or(&"unknown".to_string()));
    ...
    println!("  Result: {}", if result3.success { "✅ Success" } else { "❌ Failed" });
    println!("  Analysis: {}", context3.get_parameter("analysis").unwrap_or(&"unknown".to_string()));
    println!("  Density: {:.2}", grid3.count(|t| t.is_floor()) as f32 / (grid3.width() * grid3.height()) as f32);
}
