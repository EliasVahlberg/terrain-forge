use terrain_forge::{Grid, Rng, pipeline::*};
...
fn demo_conditional_pipeline() {
    ...
    println!("  Pipeline execution: {}", if result.success { "✅ Success" } else { "❌ Failed" });
    if let Some(msg) = result.message {
        println!("  Message: {}", msg);
    }
    
    println!("  Floor tiles: {}", grid.count(|t| t.is_floor()));
    println!("  Density status: {}", context.get_parameter("density_status").unwrap_or(&"unknown".to_string()));
...
fn demo_pipeline_templates() {
    ...
    println!("  Execution: {}", if result.success { "✅ Success" } else { "❌ Failed" });
    println!("  Generation type: {}", context.get_parameter("generation_type").unwrap_or(&"unknown".to_string()));
    println!("  Floor tiles: {}", grid.count(|t| t.is_floor()));
    println!();
}
